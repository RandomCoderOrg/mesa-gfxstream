...
static void initialize_api(void)
{
    ...
    api.ready = api.allocate != NULL && api.release != NULL && api.describe != NULL &&
                api.lock != NULL && api.unlock != NULL && api.send != NULL && api.recv != NULL;
...
int AHardwareBuffer_allocate(const AHardwareBuffer_Desc *desc, AHardwareBuffer **out_buffer)
{
    ...
    if (trace_enabled) {
        fprintf(stderr,
                "ahb-wrapper: stage=allocate_call wrapper=%p target=%p width=%u height=%u usage=0x%" PRIx64 "\n",
                (void *)AHardwareBuffer_allocate, (void *)api.allocate,
                desc->width, desc->height, desc->usage);
        fflush(stderr);
    }
    if (api.allocate == AHardwareBuffer_allocate) {
        fprintf(stderr, "ahb-wrapper: refusing recursive AHardwareBuffer_allocate target\n");
        return -ELOOP;
    }
...
void AHardwareBuffer_describe(const AHardwareBuffer *buffer, AHardwareBuffer_Desc *out_desc)
{
    if (buffer != NULL && out_desc != NULL && ensure_api() == 0) {
        if (trace_enabled) {
            fprintf(stderr,
                    "ahb-wrapper: stage=describe_call wrapper=%p target=%p buffer=%p\n",
                    (void *)AHardwareBuffer_describe, (void *)api.describe,
                    (const void *)buffer);
            fflush(stderr);
        }
        if (api.describe == AHardwareBuffer_describe) {
            fprintf(stderr, "ahb-wrapper: refusing recursive AHardwareBuffer_describe target\n");
            return;
        }
                         ...
                         const ARect *rect, void **out_virtual_address)
{
    ...
    if (trace_enabled) {
        fprintf(stderr, "ahb-wrapper: stage=lock_call target=%p buffer=%p usage=0x%" PRIx64 "\n",
